...
const AccountChart = ({ transactions }) => {
  ...
  const filteredData = useMemo(() => {
    const range = DATE_RANGES[selectedDateRange];
    const now = new Date();
    const startDate = range.days
      ? startOfDay(subDays(now, range.days))
      : startOfDay(new Date(0));

    //   filtering trnsactions within date range
    ...
    const groupedTransactions = filtered.reduce((acc, t) => {
      ...
      // adding new date object to accumulator
      if (!acc[date]) {
        acc[date] = {
          date,
          income: 0,
      ...
      // updating the income/expense for the current date
      if (t.type === "INCOME") {
  ...
  //   caluclating totals for the selected date range
